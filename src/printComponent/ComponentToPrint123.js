...
//var FMLIST = [];
export default class ComponentToPrint123 extends React.PureComponent {
  constructor(props) {
    super(props);
  }
  ...
  componentDidMount() {
    axios
      .get("http://localhost:8080/UKP/rest/endpoints/GetReportWithElements", {
        params: {
          GenInfoID: "6CC5E9F0-DC89-4B53-B4D5-DA49D167617A",
        },
      })
      .then((res) => {
        //  FMLIST=res.data.FmList;
        //console.log(res)
        //this.FmList = res.data.FmList
        //  this.setState({FmList:res.data.FmList})
        //this.setState(FmList=>[this.FmList,res.data.FmList])
        this.setState({ FmList: res.data.FmList });
      ...
      <div style={{margin:'16px'}}>
        ...
        <table width="100%">
          <tr>
            ...
            <td width="5%">Rate (Rs): t163</td>
            <td width="5%">Amount (Rs): t1765.53</td>
          </tr>
        </table>
        <p align="center">
          ---------------------------------------------------------------------------------------------------------------------------------------------
